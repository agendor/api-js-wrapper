/**
 * This is an example of how to use Agendor wrapper to create a Person, a Deal related to
 * this Person and a Task related to the Deal on Agendor.
 *
...
$(document).on('submit','#contactForm', function(){
    /**
     * In this example, we're inserting a Person, a Deal and a Task.
     * The Person will be inserted with values from the contact form.
     * The Deal and the Task will be inserted with some hard-coded values.
     *
     * and the deal's description will be the value of the 'message' field from the contact form.
     *
     * - You can see all possible fields(required and optional) for person, deal and task here:
     * - https://api.agendor.com.br/#people
     * - https://api.agendor.com.br/#deals
     * - https://api.agendor.com.br/#tasks
     *
     *
     * Create a task object with all desired values...
     */
    var task = {
     ...
     * The wrapper will call Agendor API and insert the person, the deal and the task.
     */
    agendor.task.add(task, function(result){
        if(result.error){
            //Something bad happened, 'result.error' is a message explaining what was wrong.
        }else{
            //The 'result' variable will be a task object with all fields inserted.
        }
    });
